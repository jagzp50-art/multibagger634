...
- yFinance-only data layer (`lite/data.py`) — no API keys, no Redis/Celery, with a **Data Quality card** exposing per-field fundamentals coverage (the yFinance bottleneck made visible: missing ROCE/CFO fields hold scores down via the confidence dampener)
- **Discovery Engine** (`lite/discovery.py` + Discovery screen): Emerging Leaders ranked by discovery score (35% RS rank · 25% RS acceleration (1M−3M percentile) · 20% revision proxy · 20% momentum, confidence-dampened)
- 6-screen React dashboard: Dashboard · Screener · Elite Picks · Discovery · Portfolio · Backtest
- 5-factor regime-aware scoring (Quality/Growth/Momentum/Valuation/Risk) with a **dedicated revision-proxy factor** (EPS accel + revenue accel + margin expansion + CFO growth, 10% of the composite with its own attribution bar), **data-confidence dampening** (partial fundamentals can't rank high), multi-factor RS (1/3/6/12M percentiles), **institutional quality** (5y stability), earnings-revision proxy + **quality-of-earnings** (CFO/PAT, CFO growth, **accrual ratio**), sector rotation (RS + growth + breadth + momentum), **Kelly-Lite 2.0** sizing (score × quality ÷ volatility × drawdown-penalty, per-stock vol/max-DD persisted every scan) with **sector caps** + **factor-exposure** + **Portfolio Risk** readouts (diversification-adjusted portfolio vol, weighted max drawdown, HHI concentration, risk grade — on Dashboard + Portfolio), 100-Bagger Detector with MB v3 (incl. 5y **compounder** + **reinvestment scores**), one-click backtest + **walk-forward validation** (3×12-month folds with per-fold **universe size** + **pre-PIT flags**) + **Sortino/turnover/Universe-EW benchmark** + a realistic **transaction-cost model** (STT/brokerage/GST/SEBI/slippage, 0.25%/side floor) + **survivorship guard** (no backfilled listings)
...
- Explainable + self-measuring: **factor attribution** per stock (click any Elite Pick row), **alpha decay** (7/30/90/180-day forward returns after signals), **factor IC** (which factor predicts returns, per regime, with learned weight tilt), **market breadth** (% above 20/50/200-DMA + health score), and a **watchlist intelligence** feed (RS leaders, score surges, MB elites, top sectors)

...
- **Point-in-time by design**: every scan snapshots universe membership (`universe_history`) and fundamentals as known that day (`fundamentals_history`) — backtests and factor research never use today's ROE for a 2021 test; market breadth adds 52-week **new highs/lows**; a long-format **`factor_scores`** table (symbol × scan_date × factor) is written every scan so future factor research has clean panel data
